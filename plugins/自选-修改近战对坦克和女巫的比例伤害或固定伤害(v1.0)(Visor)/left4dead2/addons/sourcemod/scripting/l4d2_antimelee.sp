...
public Action TankOnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
	...
	if(g_fMeleeTankDamage != 0.0)
		SDKHooks_TakeDamage(victim, inflictor, attacker, g_fMeleeTankDamage > 1.0 ? g_fMeleeTankDamage : g_fMeleeTankDamage * GetEntProp(victim, Prop_Data, "m_iMaxHealth"), damagetype, weapon, damageForce, damagePosition);
	
	if(g_fMeleeTankDamage == 0.0)
		PrintHintText(attacker, "坦克免疫近战伤害.");
	else
		PrintHintText(attacker, "你的近战对坦克造成了%d点伤害.", RoundFloat(g_fMeleeTankDamage > 1.0 ? g_fMeleeTankDamage : g_fMeleeTankDamage * GetEntProp(victim, Prop_Data, "m_iMaxHealth")));
...
public Action WitchOnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
	...
	if(g_fMeleeWitchDamage != 0.0)
		SDKHooks_TakeDamage(victim, inflictor, attacker, g_fMeleeWitchDamage > 1.0 ? g_fMeleeWitchDamage : g_fMeleeWitchDamage * GetEntProp(victim, Prop_Data, "m_iMaxHealth"), damagetype, weapon, damageForce, damagePosition);

	if(g_fMeleeWitchDamage == 0.0)
		PrintHintText(attacker, "女巫免疫近战伤害.");
	else
		PrintHintText(attacker, "你的近战对女巫造成了%d点伤害.", RoundFloat(g_fMeleeWitchDamage > 1.0 ? g_fMeleeWitchDamage : g_fMeleeWitchDamage * GetEntProp(victim, Prop_Data, "m_iMaxHealth")));
	return Plugin_Handled;
}
